...
1. 시간 이벤트
- 문제 : 주문 시간이 초과되어 홈 화면으로 이동한 뒤에도 계속 일정 주기로 "시간 초과"의 토스트 메시지가 반복 출력되는 문제 발생
- 원인 : 메인 화면이 백그라운드로 이동한 이후에도 카운트다운 로직이 정지되지 않아, 타이머 이벤트가 계속 실행
- 해결 : onPause()로 해결
<br> - Activity 생명주기에 맞춰 'onPause()'에서 카운트다운 정지
<br> - 'onResume()'에서 화면 다시 활성화될 때 카운트다운 재시작
- 개선 효과
<br> - 화면 전환 이후 불필요한 토스트 메시지 반복 출력이 사라지고, 주문 흐름이 초기화되는 과정이 더 안정적으로 동작하도록 개선
